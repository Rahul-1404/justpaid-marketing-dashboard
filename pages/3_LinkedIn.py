...
def load_li_qoq():
    try:
        ...
        curr_df = get_posts_by_date_range(cs, ce, platform="LinkedIn")
        prev_df = get_posts_by_date_range(ps, pe, platform="LinkedIn")
        return curr_df, prev_df, quarter_label(cy, cq), quarter_label(py, pq)
    except Exception:
        return pd.DataFrame(), pd.DataFrame(), "", ""


qoq_curr, qoq_prev, curr_lbl, prev_lbl = load_li_qoq()
...
if has_any:
    ...
    <div style="padding: 1.5rem 0 0.5rem;">
        <span style="font-size: 1.2rem; font-weight: 800; color: {li_color};">{section_title}</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    qoq_metrics = [
        ...
        ("Total Reactions", "total_likes", False),
    ...
    if has_both:
        ...
        with tcol_left:
            ...
            if not qoq_curr.empty:
                ...
                cols_show = [c for c in ["title", "post_type", "views", "likes", "comments"] if c in top_curr.columns]
        ...
        with tcol_right:
            ...
            if not qoq_prev.empty:
                ...
                cols_show = [c for c in ["title", "post_type", "views", "likes", "comments"] if c in top_prev.columns]
        ...
        fig = content_type_breakdown(active_df, platform="LinkedIn")
        st.plotly_chart(fig, use_container_width=True, key="li_qoq_content_breakdown")
        ...
        cols_show = [c for c in ["title", "post_type", "views", "likes", "comments"] if c in top_df.columns]
