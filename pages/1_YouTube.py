...
from utils.metrics import (
    format_number, calculate_posting_frequency,
...
def load_yt_qoq():
    try:
        ...
        curr_df = get_posts_by_date_range(cs, ce, platform="YouTube")
        prev_df = get_posts_by_date_range(ps, pe, platform="YouTube")
        return curr_df, prev_df, quarter_label(cy, cq), quarter_label(py, pq)
    except Exception:
        return pd.DataFrame(), pd.DataFrame(), "", ""


qoq_curr, qoq_prev, curr_lbl, prev_lbl = load_yt_qoq()
...
if has_any:
    ...
    <div style="padding: 1.5rem 0 0.5rem;">
        <span style="font-size: 1.2rem; font-weight: 800; color: {yt_color};">{section_title}</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    if has_both:
        ...
        with tcol_left:
            ...
            if not qoq_curr.empty:
                ...
                st.dataframe(top_curr[cols_show].rename(columns={
                    "title": "Title", "post_type": "Type", "views": "Views", "likes": "Likes",
                }), use_container_width=True)
        with tcol_right:
        ...
        fig = content_type_breakdown(active_df, platform="YouTube")
        st.plotly_chart(fig, use_container_width=True, key="yt_qoq_content_breakdown")
        ...
        cols_show = [c for c in ["title", "post_type", "views", "likes"] if c in top_df.columns]
